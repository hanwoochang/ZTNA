...
- **4주차: 웹 대시보드 프로젝트 초기 세팅 및 RBAC(등급제) DB 설계**
  - React.js (또는 Next.js) 기반 프로젝트 생성
  - 관리자용 정책 서버 API 연동 (전체 유저, 기기, 로그 데이터 조회)
  - 일반 직원, 부서별(인사/보안 등), 최고 관리자로 계정 등급을 나누는 역할 기반 접근 제어(RBAC) 데이터베이스 설계 및 백엔드 적용
...
- **7주차: 관리자 능동 제어 시스템 및 권한 관리 구현**
  - 대시보드 내 특정 기기 '강제 차단(Revoke)' 버튼 구현
  - 관리자가 차단 시, 모바일 앱의 `useHeartbeat`가 이를 즉시 감지하여 강제 로그아웃 시키는 양방향 통신 테스트
  - 웹 대시보드에서 계정별 등급(일반/부서별/최고관리자)을 부여하고, 모바일 앱 기밀 게시판/일정 삭제 권한 등과 세밀하게 연동
