...
content = content.replace(
  ...
  'type="email" placeholder="Admin Email" value={email} onChange={e => setEmail(e.target.value)} style={{ boxSizing: \'border-box\' }}'
);
content = content.replace(
...
content = content.replace(/<h3 className="text-ink text-2xl font-bold tracking-tight">?ÑÏÇ¨ ?ÑÏßÅ??Î™©Î°ù<\/h3>/g, '<h2 className="text-2xl font-bold text-ink">?ÑÏÇ¨ ?ÑÏßÅ??Î™©Î°ù</h2>');
content = content.replace(/<h3 className="text-ink text-2xl font-bold tracking-tight">?®ÎßêÍ∏??êÏÇ∞ Î™©Î°ù<\/h3>/g, '<h2 className="text-2xl font-bold text-ink">?®ÎßêÍ∏??êÏÇ∞ Î™©Î°ù</h2>');

// 7. Re-add Charts (Middle Section)
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        ...
        <div className="bg-card border border-hairline rounded-md flex items-stretch shadow-sm">
          ...
          <div className="flex-1 border-r border-hairline flex justify-between items-center" style={{ padding: '24px' }}>
            <div>
              <h3 className="text-body text-sm font-bold mb-4">Í∞Ä???ÑÏßÅ????/h3>
              <p className="text-4xl font-light text-primary">
                {stats.totalUsers} <span className="text-base font-medium text-muted">Î™?/span>
              </p>
            </div>
          ...
          <div className="flex-1 border-r border-hairline flex justify-between items-center" style={{ padding: '24px' }}>
            <div>
              <h3 className="text-body text-sm font-bold mb-4">?±Î°ù???®ÎßêÍ∏???/h3>
              <p className="text-4xl font-light text-ink">
                {stats.totalDevices} <span className="text-base font-medium text-muted">?Ä</span>
              </p>
            </div>
            <div className="rounded-full border border-hairline flex items-center justify-center bg-canvas" style={{ width: '64px', height: '64px', flexShrink: 0 }}>
          ...
          <div className="flex-1 flex justify-between items-center" style={{ padding: '24px' }}>
            <div>
              <h3 className="text-body text-sm font-bold mb-4">Í∏àÏùº ÎπÑÏ†ï???ëÍ∑º Ï∞®Îã®</h3>
              <p className="text-4xl font-light text-semantic-error">
                {stats.deniedToday} <span className="text-base font-medium text-muted">Í±?/span>
              </p>
            </div>
            <div className="rounded-full border border-hairline flex items-center justify-center bg-canvas" style={{ width: '64px', height: '64px', flexShrink: 0 }}>
      ...
      <div style={{ display: 'flex', gap: '24px' }}>
        <div className="bg-card border border-hairline rounded-md shadow-sm flex flex-col" style={{ flex: '6' }}>
          <div className="border-b border-hairline" style={{ padding: '20px 24px' }}>
            <h3 className="text-ink text-base font-bold">?úÍ∞Ñ?ÄÎ≥??âÍ∑† ?ÑÌóò???∏Î†å??(ÏµúÍ∑º 24?úÍ∞Ñ)</h3>
          </div>
          <div className="flex-1" style={{ padding: '24px', height: '300px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={trend} margin={{ top: 5, right: 30, left: -20, bottom: 5 }}>
                ...
                />
                <Line type="monotone" dataKey="avgRisk" stroke="#f54e00" strokeWidth={3} dot={{ r: 4, fill: '#f54e00', strokeWidth: 0 }} activeDot={{ r: 6 }} name="?âÍ∑† ?ÑÌóò?? />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-card border border-hairline rounded-md shadow-sm flex flex-col" style={{ flex: '4' }}>
          <div className="border-b border-hairline" style={{ padding: '20px 24px' }}>
            <h3 className="text-ink text-base font-bold">?îÏ£º???∏Î¨º TOP 5 (?ÑÏ†Å ?ÑÌóò??</h3>
          </div>
          <div className="flex-1" style={{ padding: '24px', height: '300px' }}>
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={topRisky} layout="vertical" margin={{ top: 5, right: 30, left: 10, bottom: 5 }}>
                ...
                />
                <Bar dataKey="totalRisk" fill="#f54e00" radius={[0, 4, 4, 0]} barSize={32} name="?ÑÏ†Å ?ÑÌóò??>
...
console.log('App.tsx successfully reconstructed!');
